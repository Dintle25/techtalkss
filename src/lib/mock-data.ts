...
// In-memory registration store (resets on server restart)
...
export async function createRegistration(data: {
    ...
    if (duplicate) {
        throw new Error(`${data.attendeeEmail} is already registered for this talk.`)
    }

    const talk = talks.find((t) => t.id === data.talkId)
    if (!talk) {
        throw new Error(`Talk with id ${data.talkId} not found.`)
    }
